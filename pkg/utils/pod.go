...
var EventsToCollect = []string{"instructions", "cycles"}
...
func CGroupPath(pod *v1.Pod, container *v1.ContainerStatus) (string, error) {
	k8sPath := "/sys/fs/cgroup/kubepods.slice/"
	uid := strings.ReplaceAll(string(pod.UID), "-", "_")
	path := filepath.Join(k8sPath, "kubepods-pod"+uid+".slice", ContainerId(container))
	return path, nil
...
func CGroupDir(pod *v1.Pod, container *v1.ContainerStatus) (string, error) {
	k8sPath := "/sys/fs/cgroup/kubepods.slice/"
	uid := strings.ReplaceAll(string(pod.UID), "-", "_")
	path := filepath.Join(k8sPath, "kubepods-pod"+uid+".slice")
	return path, nil
}

func CGroupContainerPath(CGroupDir string, container *v1.ContainerStatus) (string, error) {
	path := filepath.Join(CGroupDir, "/", ContainerId(container))
	return path, nil
}
